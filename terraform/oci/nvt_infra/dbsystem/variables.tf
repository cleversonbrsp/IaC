...
# ========================================
variable "subnet_id" {
  description = "OCID of the subnet for the DB System"
  type        = string
}
...
variable "nsg_ids" {
  description = "List of NSG OCIDs to attach to the DB System (optional)"
  type        = list(string)
  default     = []
}
...
variable "availability_domain" {
  description = "Availability Domain for storage (e.g., FOjF:SA-SAOPAULO-1-AD-1)"
  type        = string
}
...
variable "backup_id" {
  description = "OCID of the backup to restore from (required if source_type = BACKUP)"
  type        = string
  default     = ""
}
