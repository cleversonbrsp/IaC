resource "oci_psql_db_system" "postgresql_db_system" {
  ...
  network_details {
    ...
    nsg_ids                    = var.create_network_resources ? [oci_core_network_security_group.postgresql_nsg[0].id] : var.nsg_ids
    primary_db_endpoint_private_ip = var.primary_db_endpoint_private_ip
    subnet_id                      = var.create_network_resources ? oci_core_subnet.postgresql_subnet[0].id : var.subnet_id
  }
